...
func (s *Store) MapData(ctx context.Context, since time.Time, includeInsincere, includeMissing bool, bbox *geo.BBox) (*MapData, error) {
	...
	select 
		sightings.id sighting_id, (transpired = 'sighted' OR to_exist = 1) as does_exist, max(observed_at) as observed_at, sightings.image_url, sightings.description, count(sightings.id),
	...
	group by loc.id
	having ($7 or (transpired = 'sighted' or to_exist = 1))
	...
	for rows.Next() {
		...
		m.Location.PlusCode = geo.GetPlusCode(m.Location.Lat, m.Location.Lon)
